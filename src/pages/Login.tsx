...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      setLoading(false);
      toast({ title: "Error al iniciar sesiÃ³n", description: error.message, variant: "destructive" });
      return;
    }
    ...
    <div className="flex min-h-screen items-center justify-center section-padding py-12">
      <div className="w-full max-w-sm">
        ...
        <div className="glass-card rounded-2xl p-8">
          <h1 className="mb-1 font-display text-2xl font-bold">Iniciar sesiÃ³n</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input id="password" type={showPassword ? "text" : "password"} placeholder="â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢" value={password} onChange={(e) => setPassword(e.target.value)} required className="bg-background/50 pr-10" />
            ...
            <Button type="submit" className="w-full" size="lg" disabled={loading}>
              {loading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Entrando...</> : checkoutIntent ? "Entrar y pagar" : "Entrar"}
            </Button>
          </form>

          <p className="mt-6 text-center text-sm text-muted-foreground">
            Â¿No tienes cuenta?{" "}
            <Link to={checkoutIntent ? "/registro?checkout=1" : "/registro"} className="font-medium text-primary hover:underline">RegÃ­strate</Link>
          </p>
        </div>
      </div>
    </div>
